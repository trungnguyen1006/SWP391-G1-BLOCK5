...
(function() {
   ...
   * Initiate Datatables
   */
  const datatablesInit = select('.datatablesInit', true)
  datatablesInit.forEach(datatable => {
...
const datatablesReceptionists = select('.datatable-receptionists', true)
     datatablesReceptionists.forEach(datatable => {
     new simpleDatatables.DataTable(datatable, {
         searchable: true,
         fixedHeight: false,
       perPageSelect: [5, 10, 15, ["All", -1]],
       columns: [
           ...
           { select: 5, sortable: true, searchable: true },
